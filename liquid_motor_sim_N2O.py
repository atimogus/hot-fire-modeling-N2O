...
N_ORFICE_OX = 8
N_ORFICE_F = 8
DRY_WEIGHT = 12 #kilograms (wirhout )
...
def rocketpy_sim():
    ...
    density_f = PropsSI('D', 'P', pressure_l, 'T', temp, FUEL)
    oxidizer_liq = rp.Fluid(
        name="Liquid N2O",
        density=density_l, # kg/m3
        )
    oxidizer_gas = rp.Fluid(
        name="Gas N2O",
        density=density_v, # kg/m3
    )
    ...
    #define tank
    tanks_geometry = rp.CylindricalTank(
    ...
    example_liquid_motor.add_tank(
        tank=fuel_tank,
        position=0.5 + 1.5*tank_height, # meters
    )
    ...
    #dedfiniranje uvjeta lansiranja
    env = rp.Environment(
        latitude=52.020472,
        longitude=-0.878609,
    )
    ...
    example_rocket.add_motor(example_liquid_motor, position=4)
